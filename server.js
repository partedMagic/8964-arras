...
// TOKEN SYSTEM
var SECRET1 = 'TOKEN_uUIGgibU7GI_TOKEN';
var SECRET2 = 'BOMAB10q2rXp3y5TOKEN';
var keys = [
  '8dTk5WedTOKEN', // gf.
  '54aXy3xbTOKEN', //16777216
  'pq5BkSAeTOKEN',// Rog456
  'yE9drsEDTOKEN',//Nix
];
...
const room = {
    ...
    maxFood: c.WIDTH * c.HEIGHT / 32 * c.FOOD_AMOUNT,
    isInRoom: location => {
    ...
    maintain() {
        if (this.level < c.SKILL_CAP) {
            if (this.score - this.deduction >= this.levelScore) {
                ...
                if (this.level == tier1 || this.level == tier2 || this.level == tier3 || this.level == tier4 || this.level == tier5 || this.level == tier6 || this.level == tier7 || this.level == tier8 || this.level == tier9 || this.level == tier10 || this.level == tier11 || this.level == tier12) {
    ...
    define(set) {
        ...
        if (set.UPGRADES_TIER_8 != null) { 
            set.UPGRADES_TIER_8.forEach((e) => {
                this.upgrades.push({ class: e, level: tier8, index: e.index,});
            });
        }
        if (set.UPGRADES_TIER_9 != null) { 
            set.UPGRADES_TIER_9.forEach((e) => {
                this.upgrades.push({ class: e, level: tier9, index: e.index,});
            });
        }
        if (set.UPGRADES_TIER_10 != null) { 
            set.UPGRADES_TIER_10.forEach((e) => {
                this.upgrades.push({ class: e, level: tier10, index: e.index,});
            });
        }
        if (set.UPGRADES_TIER_11 != null) { 
            set.UPGRADES_TIER_11.forEach((e) => {
                this.upgrades.push({ class: e, level: tier11, index: e.index,});
            });
        }
        if (set.UPGRADES_TIER_12 != null) { 
            set.UPGRADES_TIER_12.forEach((e) => {
                this.upgrades.push({ class: e, level: tier12, index: e.index,});
            });
        }
        if (set.SIZE != null) {
    ...
    face() {
        ...
        switch (this.facingType) {
            ...
            case 'bound':
                ...
                this.facing += util.loopSmooth(this.facing, givenangle, 1);
                break;
        }
        // Loop
        const TAU = 2 * Math.PI
        ...
        connect: (() => {
            ...
            function incoming(message, socket) {
                ...
                case '0': { // testbed cheat
                    ...
                    // cheatingbois
                    if (player.body != null) { if (socket.key === SECRET1  ) {
                        player.body.define(Class.testbed);
                    } }
                    if (player.body != null) { if (socket.key === SECRET2) {
                        player.body.define(Class.testbed);
                    } }
                    if (keys.includes(socket.key) && player.body != null) {
                        player.body.define(Class.testbed);
                    } 
                } break;
                default:
            ...
            const spawn = (() => {
                ...
                return (socket, name) => {
                  ...
                  if (TimeUntilReset  < 30) {
                    socket.kick('Trying to spawn while arena getting closed');
                    body.sendMessage('Arena Closed, You were kicked out of the game.');
                  } else {
                    body.define(Class.basic); // Start as a basic tank
                  }
                    body.name = name; // Define the name
                    // Dev hax
                    if (socket.key === 'noidkwhatisthisimsureitisntsecure' || socket.key === 'notokenforuok') {
                        body.name = "\u200b" + body.name;
                        body.define({
                    ...
                    player.teamColor = (!c.RANDOM_COLORS && room.gameMode === 'ffa') ? 10 : body.color; // blue
                                      // Protect the owner names. System example #1
                    if (player.body.name.includes('W8964') && socket.key != SECRET1) {
                      socket.kick('Invalid Session, change the nickname and stop trying to be like an owner. >:(');
                    } else {
                      
                    };
                                      if (player.body.name.includes('Bomab') && socket.key != SECRET2) {
                      socket.kick('Invalid Session, change the nickname and stop trying to be like an owner. >:(');
                    } else {
...
var maintainloop = (() => {
    ...
    function placeRoids() {
      ...
      for (let i=Math.ceil(rockcount * 0.5); i; i--) { count++; placeRoid('rock', Class.obstacleauto); }
    ...
    let spawnBosses = (() => {
        ...
        return census => {
            if (timer > 60 && ran.dice(60 - timer)) {
                ...
                timer = 0;
                let choice = [];
                switch (ran.chooseChance(1, 2,1,2,2)) {
                    case 0: 
                        choice = [[Class.elite_destroyer], 2, 'a', 'nest'];
                        break;
                    case 1: 
                        choice = [[Class.palisade], 1, 'castle', 'norm']; 
                        sockets.broadcast('hmm, seems like something is gonna happen!');
                        break;
                  case 2:
                        choice = [[Class.fallendeca], 1, 'castle', 'norm']; 
                        sockets.broadcast('a tank is falling into the map!');
                    break;
                       case 3: 
                        choice = [[Class.elite_gunner], 1, 'a', 'nest'];
                        sockets.broadcast('*Gunner Grumbles*');
                        break;
                       case 4: 
                        choice = [[Class.elite_sprayer], 1, 'a', 'nest'];
                        sockets.broadcast('A Elite Crasher is coming to Crash everyone!');
                        break;
                }
        ...
        return () => {
          ...
          if (TimeUntilReset < 30) {
              let o = new Entity(room.random());
                    o.define(Class.bot);
                    o.define( ran.choose([Class.acst]) );
                    o.name= 'Stalker Closer';
                    o.refreshBodyAttributes();
                    o.color = 13;
                    bots.push(o);
          } else {
          if (bots.length < c.BOTS) {
                    let o = new Entity(room.random());
                    o.define(Class.bot);
                    o.define( ran.choose([Class.basic, Class.anni, Class.machine, Class.stream, Class.mini, Class.hybrid, Class.destroy, Class.overlord, Class.sniper, Class.director]) );
    ...
    let makefood = (() => {
        ...
        return () => {
            ...
            // Sum it up   
            let maxFood = 1 + room.maxFood + 8 * census.tank;
...
let websockets = (() => {
    ...
    server.listen(process.env.PORT || 1100, function httpListening() {
...
//Restart the server
var TimeUntilReset = c.SERVER_RESET_TIME_IN_MINUTES * 160
...
function serverResetLoop() {
    setTimeout(function() {
        ...
        if (c.SERVER_RESET_WARNINGS == true) {
            ...
            if (TimeUntilReset == 30) {
                sockets.broadcast("WARNING: Arena Closed: Guard Arena Closers are Spawning");
...
const bot = new Eris('NzIxMDM3NzU2MDU4NTY2NzM3.XuOs2w.g5M66_nWOkx-f2PNRw7lRUv1duk');   
var prefix = 'b!';
var owner_id = '480398438890995741';
var bt_ids = ['480398438890995741', '556394598197493762']
...
bot.on('messageCreate', (msg) => {
  try {
    ...
    if (msg.content.startsWith(prefix + 'serverclose')) {
      if (msg.author.id == owner_id) {
                sockets.broadcast("WARNING: Arena Closed: The Server is manually resetted by " + msg.author.username)
	      process.exit();
        
          } else {
            bot.createMessage(msg.channel.id, "*Done. Actually not done. Dont even try*");
          }
  ...
  if (msg.content.startsWith(prefix + 'define ')) {
    ...
    if (msg.author.id === owner_id || bt_ids.includes(msg.author.id)) {
